            ...
            <div className="mx-auto max-w-7xl py-24 sm:px-2 sm:py-32 lg:px-4">
                <div className="mx-auto max-w-2xl px-4 lg:max-w-none">
                    <header className="max-w-3xl">
                        <h2 id="benefits-heading" className="text-4xl font-bold tracking-tight text-white">
                    ...
                    <ul className="mt-16 grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-3" role="list" aria-label="Beneficios de Aliworld">
                        {incentives.map((incentive) => (
                            <li key={incentive.name} className="sm:flex lg:block" tabIndex={0} aria-label={incentive.name}>
                                <div className="sm:shrink-0">
                                    <incentive.icon className="w-12 h-12 text-white" aria-hidden="true" />
                                </div>
                                <div className="mt-4 sm:ml-6 sm:mt-0 lg:ml-0 lg:mt-6">
                                    <h3 className="text-lg font-bold text-white" id={`benefit-title-${incentive.name.replace(/\s+/g, '-')}`}>{incentive.name}</h3>
                                    <p className="mt-2 text-base text-gray-100" aria-describedby={`benefit-title-${incentive.name.replace(/\s+/g, '-')}`}>{incentive.description}</p>
                                </div>
                            </li>
                        ))}
                    </ul>
